...
public:
	...
	Wizard& operator=(int k) {

			return *this;
		}
	void operator[](string s){
		for (unsigned int i = 0; i < spells.size(); i++) {
			if (s.find(spells[i]->attributes[0][0]->stringval)!=std::string::npos ) {
	...
	Wizard& operator=(int x) {
			...
			if (DmgOrHeal == 1) {
				if(attacker=="Gryffindor"){
				 ...
				 ::NotPlaying.attributes[0][2]->intval -= x;
			}else {
				int t;
				t=::Playing.attributes[0][2]->intval + x;
				if(t>::Playing.maxhealth){
					::Playing.attributes[0][2]->intval=::Playing.maxhealth;
				}else ::Playing.attributes[0][2]->intval += x;
			}
			return *this;
		}
};
...
string get_house(Wizard w) {
	return w.attributes[0][1]->stringval;
}
string get_name(Wizard w) {
	return w.attributes[0][0]->stringval;
}
bool has_wand(Wizard w){
	if(w.hasWand==1)return true;
	return false;
}


//Start/End of programm
#define BEGIN_GAME int main(){cout<<GREEN<<"-------------------------------------Harry Potter the Game-------------------------------------"<<RESET<<"\n\n";
...
#define START [](){
#define DAMAGE DmgOrHeal = 1;DMGHEAL::
#define ATTACKER Playing=
#define DEFENDER NotPlaying=
#define HEAL DmgOrHeal = 2;DMGHEAL::
#define END ;}
#define GET_HP(p) get_hp(p 1)
#define GET_HOUSE(p) get_house(p 1)
#define GET_NAME(p) get_name(p 1)
#define HAS_WANDp has_wand(p 1)
#define EQUIP ;equip(
#define a wand1);
#define _ ---wand2);
